...
safe_bump = """
        function bumpDailyQuest(type, amount) {
            amount = amount || 1;
            let data = loadData();
            ...
            if (newlyClaimed) {
                if (typeof triggerToast !== 'undefined') triggerToast('Quest Complete!', 'You earned ' + newlyClaimed.xp + ' XP.');
                if (document.getElementById('view-games') && document.getElementById('view-games').classList.contains('active') && typeof renderDailyQuests !== 'undefined') {
                    renderDailyQuests();
                }
            }
        }
...
if start != -1:
    end = content.find("function awardBonusXp(", start)
    if end != -1:
